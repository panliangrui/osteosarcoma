...
class CrossAttentionBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # self.sattn = SelfAttention(dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.)
        self.norm1 = norm_layer(dim)
        self.sattn = SelfAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
    ...
    def forward(self, x):
        ...
        x = x + self.drop_path(self.sattn(self.norm1(x)))
        ...
        for d in range(num_branches):
            ...
            for i in range(depth[d]):
                tmp.append(
                    ...
                    #     dim=dim[d], input_resolution= input_resolution, num_heads=num_heads[d], window_size=window_size,
...
class Net(nn.Module):
    def __init__(self, embed_dim,  num_classes):
        ...
        self.conv1 = nn.Conv1d(in_channels=embed_dim,  out_channels=num_classes, kernel_size=3, stride=2)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=(224, 224), patch_size=(8, 16), in_chans=3, num_classes=1000, embed_dim=(192, 384), depth=([1, 3, 1], [1, 3, 1], [1, 3, 1]),
                 ...
                 drop_path_rate=0., hybrid_backbone=None, ape=False, norm_layer=nn.LayerNorm, multi_conv=False):
        super().__init__()
        ...
        for i in range(self.num_branches):
            self.net = Net(embed_dim=embed_dim[i], num_classes=num_classes)
    ...
    def reset_classifier(self, num_classes, global_pool=''):
        ...
        for i in range(self.num_branches):
            self.net = Net(embed_dim=self.embed_dim, num_classes=num_classes)
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_branches):
            ...
            if self.ape:
                tmp = self.patch_embed[i](x_) + self.absolute_pos_embed
            else:
